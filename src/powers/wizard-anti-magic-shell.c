...
#ifndef WIZARD_ANTI_MAGIC_SHELL_POWER_H
#define WIZARD_ANTI_MAGIC_SHELL_POWER_H
...
extern int wizard_anti_magic_shell_can_activate (power_instance_t *source, const int16_t power_level) {
    if (!source || !source->entity) { return 0;}
    return entity_has_wizard_slot(source->entity, power_level);
}

extern int wizard_anti_magic_shell_pay          (power_instance_t *source, const int16_t power_level) {
    if (!source || !source->entity) { return 0;}
    return entity_take_wizard_slot(source->entity, power_level);
}

extern void wizard_anti_magic_shell_apply        (power_instance_t *source, entity_t *entity) {
...
extern int wizard_anti_magic_shell_affect_power (power_instance_t *target) {
    if (!target) { return 0;}
    return 0; // Doesn't affect powers.
}

extern int wizard_anti_magic_shell_update       (power_t *power, power_instance_t *source) {
    if (!power || ! source) { return 0;}
    entity_t *entity = source->entity;

    if (entity) {
        power->range = 1;
        power->aoe = 0 + (4 * entity_get_wizard_level(entity));
    }
...
extern void wizard_anti_magic_shell_setup  (power_t *power) {
    ...
    power->description          = spin_read_description(select_by_game(0, 69, 69));
    power->range                = -99999;
    power->aoe                  = -99999;
    power->shape                = TARGET_SELF;
    power->cast_sound           = select_by_game(12, 12, 12);
    power->thrown_sound         = select_by_game(255, 255, 255);
    power->hit_sound            = select_by_game(223, 223, 223);
    power->actions.can_activate = wizard_anti_magic_shell_can_activate;
