...
#ifndef WIZARD_POWER_WORD_KILL_POWER_H
#define WIZARD_POWER_WORD_KILL_POWER_H
...
static int wizard_power_word_kill_update       (power_t *power, power_instance_t *source) {
    ...
    if (entity) {
        power->range = 0 + (32 * entity_get_wizard_level(entity));
        power->aoe = 80;
    }
...
extern void wizard_power_word_kill_setup  (power_t *power) {
    ...
    power->description          = spin_read_description(select_by_game(-1, 109, 109));
    power->range                = -99999;
    power->aoe                  = -99999;
    power->level                = 9;
    ...
    power->hit_sound            = select_by_game(179, 179, 179);
    power->actions.can_activate = wizard_power_word_kill_can_activate;
    power->actions.pay          = wizard_power_word_kill_pay;
    power->actions.apply        = wizard_power_word_kill_apply;
    power->actions.affect_power = wizard_power_word_kill_affect_power;
    powers_set_icon(power, select_by_game(21108, 21108, 21108));
    powers_set_cast(power, select_by_game(-1, -1, -1)); // OJFF
    powers_set_thrown(power, select_by_game(8115, 8115, 8115)); // OJFF.
    powers_set_hit(power, select_by_game(8012, 8012, 8012)); // OJFF
}
