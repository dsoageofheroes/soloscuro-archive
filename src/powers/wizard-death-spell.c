...
#ifndef WIZARD_DEATH_SPELL_POWER_H
#define WIZARD_DEATH_SPELL_POWER_H
...
extern int wizard_death_spell_can_activate (power_instance_t *source, const int16_t power_level) {
    if (!source || !source->entity) { return 0;}
    return entity_has_wizard_slot(source->entity, power_level);
}

extern int wizard_death_spell_pay          (power_instance_t *source, const int16_t power_level) {
    if (!source || !source->entity) { return 0;}
    return entity_take_wizard_slot(source->entity, power_level);
}

extern void wizard_death_spell_apply        (power_instance_t *source, entity_t *entity) {
    ...
    effect_type = 0 | EFFECT_MAGIC | EFFECT_DEATH;
}

extern int wizard_death_spell_affect_power (power_instance_t *target) {
    if (!target) { return 0;}
    return 0; // Doesn't affect powers.
}

extern int wizard_death_spell_update       (power_t *power, power_instance_t *source) {
    if (!power || ! source) { return 0;}
    entity_t *entity = source->entity;

    if (entity) {
        power->range = 0 + (120 * entity_get_wizard_level(entity));
        power->aoe = 120 + (1 * entity_get_wizard_level(entity));
    }
...
extern void wizard_death_spell_setup  (power_t *power) {
    ...
    power->description          = spin_read_description(select_by_game(0, 72, 72));
    power->range                = -99999;
    power->aoe                  = -99999;
    ...
    power->actions.can_activate = wizard_death_spell_can_activate;
    power->actions.pay          = wizard_death_spell_pay;
    power->actions.apply        = wizard_death_spell_apply;
    power->actions.affect_power = wizard_death_spell_affect_power;
    powers_set_icon(power, select_by_game(21071, 21071, 21071));
