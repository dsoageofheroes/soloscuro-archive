...
static int wizard_pebble_to_boulder_can_activate (power_instance_t *source, const int16_t power_level) {
    if (!source || !source->entity) { return 0;}
    return entity_has_wizard_slot(source->entity, power_level);
}

static int wizard_pebble_to_boulder_pay          (power_instance_t *source, const int16_t power_level) {
    if (!source || !source->entity) { return 0;}
    return entity_take_wizard_slot(source->entity, power_level);
}

static void wizard_pebble_to_boulder_apply        (power_instance_t *source, entity_t *entity) {
...
static int wizard_pebble_to_boulder_update       (power_t *power, power_instance_t *source) {
