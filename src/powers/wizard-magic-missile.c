...
static int wizard_magic_missile_can_activate (power_instance_t *source, const int16_t power_level) {
    if (!source || !source->entity) { return 0;}
    return entity_has_wizard_slot(source->entity, power_level);
}

static int wizard_magic_missile_pay          (power_instance_t *source, const int16_t power_level) {
    if (!source || !source->entity) { return 0;}
...
static int wizard_magic_missile_update       (power_t *power, power_instance_t *source) {
