...
#ifndef WIZARD_FORCE_CAGE_POWER_H
#define WIZARD_FORCE_CAGE_POWER_H
...
extern int wizard_force_cage_can_activate (power_instance_t *source, const int16_t power_level) {
    if (!source || !source->entity) { return 0;}
    return entity_has_wizard_slot(source->entity, power_level);
}

extern int wizard_force_cage_pay          (power_instance_t *source, const int16_t power_level) {
    if (!source || !source->entity) { return 0;}
    return entity_take_wizard_slot(source->entity, power_level);
}

extern void wizard_force_cage_apply        (power_instance_t *source, entity_t *entity) {
...
extern int wizard_force_cage_affect_power (power_instance_t *target) {
    if (!target) { return 0;}
    return 0; // Doesn't affect powers.
}

extern int wizard_force_cage_update       (power_t *power, power_instance_t *source) {
...
extern void wizard_force_cage_setup  (power_t *power) {
    ...
    power->description          = spin_read_description(select_by_game(0, 86, 86));
    power->range                = -99999;
    power->aoe                  = -99999;
    ...
    power->actions.can_activate = wizard_force_cage_can_activate;
    power->actions.pay          = wizard_force_cage_pay;
    power->actions.apply        = wizard_force_cage_apply;
    power->actions.affect_power = wizard_force_cage_affect_power;
    powers_set_icon(power, select_by_game(21085, 21085, 21085));
