...
extern int wizard_conjure_greater_elemental_can_activate (power_instance_t *source, const int16_t power_level) {
    if (!source || !source->entity) { return 0;}
    return entity_has_wizard_slot(source->entity, power_level);
}

extern int wizard_conjure_greater_elemental_pay          (power_instance_t *source, const int16_t power_level) {
    if (!source || !source->entity) { return 0;}
    return entity_take_wizard_slot(source->entity, power_level);
}

extern void wizard_conjure_greater_elemental_apply        (power_instance_t *source, entity_t *entity) {
...
extern int wizard_conjure_greater_elemental_affect_power (power_instance_t *target) {
    if (!target) { return 0;}
    return 0; // Doesn't affect powers.
}

extern int wizard_conjure_greater_elemental_update       (power_t *power, power_instance_t *source) {
...
extern void wizard_conjure_greater_elemental_setup  (power_t *power) {
    ...
    power->description          = spin_read_description(select_by_game(0, 82, 82));
    power->range                = -99999;
    power->aoe                  = -99999;
