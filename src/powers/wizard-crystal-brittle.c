...
static int wizard_crystal_brittle_can_activate (power_instance_t *source, const int16_t power_level) {
    if (!source || !source->entity) { return 0;}
    return entity_has_wizard_slot(source->entity, power_level);
}

static int wizard_crystal_brittle_pay          (power_instance_t *source, const int16_t power_level) {
    if (!source || !source->entity) { return 0;}
    return entity_take_wizard_slot(source->entity, power_level);
}

static void wizard_crystal_brittle_apply        (power_instance_t *source, entity_t *entity) {
...
static int wizard_crystal_brittle_affect_power (power_instance_t *target) {
    if (!target) { return 0;}
    return 0; // Doesn't affect powers.
}

static int wizard_crystal_brittle_update       (power_t *power, power_instance_t *source) {
...
extern void wizard_crystal_brittle_setup  (power_t *power) {
    ...
    power->description          = spin_read_description(select_by_game(104, 104, 104));
    power->range                = -99999;
    power->aoe                  = -99999;
    power->level                = 9;
